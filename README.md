...
对该服务做了一组对照实验 (相同参数跑两次, 以及分别只改其中一项):

| 变更 | hkey |
|---|---|
| 相同 `(imei, path, time)` 跑两次 | 完全一致 |
| 只改 `imei` | 变化 |
| 只改 `path` | 变化 |
| 只改 `time` | 变化 |

结论: hkey 是 `(path, time, imei, heybox_id)` 的**确定性函数**, 没有服务端随机数。

**推论 (重要):** `imei` 参与签名, 因此设备参数必须与账号真实设备保持一致。若使用固定 imei, 多个账号会呈现为**同一台不存在的设备**, 这是风控最容易聚类的特征。本模块优先使用抓包得到的真实设备参数, 而非硬编码值。

同样由于这四项强绑定: 取签名与发请求时的 `imei` / `path` / `time` / `heybox_id` 必须完全一致, 否则签名校验必然失败。代码中以同一变量传递, 保证不会错位。
